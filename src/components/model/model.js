...
const closeModalWindow = value => {
	value.classList.remove('popup_is-opened'); // Убираем классы, отвечающие за открытость и анимацию модального окна
...
const openModalWindow = value => {
	...
	setTimeout(() => {
		value.classList.add('popup_is-opened');
	}, 100); // Добавляем классы, что бы открыть модальное окно
